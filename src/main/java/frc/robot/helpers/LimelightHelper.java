...
public class LimelightHelper {
    ...
    public static boolean getFound() {
        NetworkTable table = NetworkTableInstance.getDefault().getTable("limelight-turret"); // Gets the table of limelight values
        NetworkTableEntry tx = table.getEntry("tv"); // Gets the Horizontal Offset From Crosshair To Target, -29.8 tp 29.8

        return (1 == (int) tx.getDouble(0));
    }
    ...
    public static double getIntakeRawY() {
        NetworkTable table = NetworkTableInstance.getDefault().getTable("limelight"); // Gets the table of limelight values
        NetworkTableEntry ty = table.getEntry("ty"); // Gets the Veritcal Offset From Crosshair To Target, -24.85 to 24.85
        double y = ty.getDouble(0.0);
        return y;
    }
